...
const sampleGraphicsData = [
  {
    srNo: '1',
    projectName: 'Logo Design',
    clientName: 'Sample Graphics Client',
    clientNumber: '1234567890',
    mailID: 'graphicsclient@gmail.com',
    amc: 'Yes',
    category: 'logo',
...
function getGraphicsData(){
  let data = JSON.parse(localStorage.getItem('accounts_graphics_data') || '[]');
  if(data.length === 0){
    localStorage.setItem('accounts_graphics_data', JSON.stringify(sampleGraphicsData));
    data = sampleGraphicsData;
  }
  return data;
}
...
function renderTable(data, tableId) {
  ...
  table.innerHTML = '';
  if(data.length === 0){
    table.innerHTML = '<tr><td colspan="' + (graphicsDisplayColumns.length + 1) + '">No records found</td></tr>';
    return;
  }

  const thead = document.createElement('thead');
  const trHead = document.createElement('tr');
  graphicsDisplayColumns.forEach(([key, label]) => {
  ...
  data.forEach((row, rowIndex) => {
    ...
    graphicsDisplayColumns.forEach(([key]) => {
...
function filterActiveProjects(data) {
...
function handleViewClick(e){
  ...
  window.location.href = `graphics-detail.html?index=${index}&type=${category}`;
}

document.addEventListener('DOMContentLoaded', () => {
  const category = getQueryParam('type') || 'total';
  const data = getGraphicsData();
  const filteredData = filterDataByCategory(data, category);
  const activeProjects = filterActiveProjects(filteredData);
  const renewals = filterRenewals(filteredData);

  renderTable(filteredData, 'table1');
  renderTable(activeProjects, 'table2');
  renderTable(renewals, 'table3');

  setupSearch('search1', 'table1', filteredData);
  setupSearch('search2', 'table2', activeProjects);
