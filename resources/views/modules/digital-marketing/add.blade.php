...
<head>
  ...
  <title>Add Digital Marketing Campaign</title>
  ...
  <div class="layout">
    ...
    <main class="content">
      <div class="topbar">
        ...
        <div class="h1">Add Digital Marketing Campaign</div>
        <div class="actions"><a class="btn" href="{{ url('/digital-marketing') }}">Cancel</a></div>
      </div>

      <form id="addForm" class="form panel" method="POST" action="{{ url('/digital-marketing/store') }}">
        @csrf
        <!-- Fields will be injected by JS -->
      </form>

      <div class="actions">
        <button class="btn link" type="button" onclick="history.back()">◀ Back</button>
        <button class="btn primary" id="submitBtn" type="submit" form="addForm">Submit</button>
      </div>
    </main>
  </div>

  <script src="{{ asset('dashboard-ui/js/app.js') }}"></script>
  <script src="{{ asset('dashboard-ui/js/digital-marketing-add.js') }}"></script>
</body>
</html>